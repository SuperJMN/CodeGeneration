    ...
    public class Expressions
    {
        ...
        private static readonly TokenListParser<LangToken, Expression> BooleanValue =
            Token.EqualTo(LangToken.True).Value((Expression) new NewConstantExpression(true))
                .Or(Token.EqualTo(LangToken.False).Value((Expression) new NewConstantExpression(false)))
            .Named("boolean");
        
        private static readonly TokenListParser<LangToken, Expression> Literal =
            Number
                .Or(Identifier)
                .Or(Token.EqualTo(LangToken.Null).Value((Expression) new NewConstantExpression(null)))
                .Or(BooleanValue)
                .Named("literal");
        ...
        {
            return new CallExpression(operatorName);
        }
    }
}