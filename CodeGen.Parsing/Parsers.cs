...
namespace CodeGen.Parsing
{
    ...
    {
        private static readonly TokenListParser<LangToken, string> Identifier =
            Token.EqualTo(LangToken.Identifier).Select(token => token.ToStringValue());

        private static readonly TokenListParser<LangToken, Expression> Indexer =
            Parse.Ref(() => Expression.OptionalOrDefault().BetweenBrackets());

        private static readonly TokenListParser<LangToken, Expression> ReferenceAccess =
            from aste in Token.EqualTo(LangToken.Asterisk).Many()
            from id in Identifier
            from indexer in Indexer.OptionalOrDefault()
            select (Expression)new ReferenceAccessItem(id, indexer, aste.Length);

        private static readonly TokenListParser<LangToken, string> Add = Token.EqualTo(LangToken.Plus)
            .Value(Operator.Add);

        private static readonly TokenListParser<LangToken, string> Subtract =
            Token.EqualTo(LangToken.Minus).Value(Operator.Subtract);

        private static readonly TokenListParser<LangToken, string> Or = Token.EqualTo(LangToken.Or).Value(Operator.Or);

        private static readonly TokenListParser<LangToken, string> And = Token.EqualTo(LangToken.And)
            .Value(Operator.And);

        private static readonly TokenListParser<LangToken, string> Multiply =
            Token.EqualTo(LangToken.Asterisk).Value(Operator.Multiply);

        private static readonly TokenListParser<LangToken, string> Divide =
            Token.EqualTo(LangToken.Slash).Value(Operator.Divide);

        private static readonly TokenListParser<LangToken, string> Modulo =
            Token.EqualTo(LangToken.Mod).Value(Operator.Module);

        private static readonly TokenListParser<LangToken, string> Lte = Token.EqualTo(LangToken.LessThanOrEqual)
            .Value(Operator.Lte);

        private static readonly TokenListParser<LangToken, string> Lt = Token.EqualTo(LangToken.LessThan)
            .Value(Operator.Lt);

        private static readonly TokenListParser<LangToken, string> Gt = Token.EqualTo(LangToken.GreaterThan)
            .Value(Operator.Gt);

        private static readonly TokenListParser<LangToken, string> Gte = Token.EqualTo(LangToken.GreaterThanOrEqual)
            .Value(Operator.Gte);

        private static readonly TokenListParser<LangToken, string> Eq = Token.EqualTo(LangToken.DoubleEqual)
            .Value(Operator.Eq);

        private static readonly TokenListParser<LangToken, string> Neq = Token.EqualTo(LangToken.NotEqual)
            .Value(Operator.Neq);

        private static readonly TokenListParser<LangToken, string> Power = Token.EqualTo(LangToken.Caret)
            .Value(Operator.Power);

        private static readonly TokenListParser<LangToken, string> Not = Token.EqualTo(LangToken.Exclamation)
            .Value(Operator.Not);

        private static readonly TokenListParser<LangToken, string> Negate =
            Token.EqualTo(LangToken.Minus).Value(Operator.Negate);

        private static readonly TokenListParser<LangToken, string> Increment =
            Token.EqualTo(LangToken.DoublePlus).Value(Operator.Increment);

        private static readonly TokenListParser<LangToken, string> Decrement =
            Token.EqualTo(LangToken.DoubleMinus).Value(Operator.Decrement);

        private static readonly TokenListParser<LangToken, string> PointerValue =
            Token.EqualTo(LangToken.Asterisk).Value(Operator.PointerValue);

        private static readonly TokenListParser<LangToken, string> PointerAddress =
            Token.EqualTo(LangToken.Ampersand).Value(Operator.PointerAddress);

        private static readonly TokenListParser<LangToken, Expression> Number = Token.EqualTo(LangToken.Number)
        ...
        private static readonly TokenListParser<LangToken, Expression> Reference =
            Token.EqualTo(LangToken.Identifier)
                .Select(token => (Expression) new ReferenceAccessItem(token.ToStringValue()));
        ...
        private static readonly TokenListParser<LangToken, Expression> Item =
            FunctionCall.Try().Or(ReferenceAccess).Or(Literal);
        ...
        private static readonly TokenListParser<LangToken, Expression> InnerTerm =
            Parse.Chain(Power, Operand, MakeBinary);

        private static readonly TokenListParser<LangToken, Expression> Term =
            Parse.Chain(Multiply.Or(Divide).Or(Modulo), InnerTerm, MakeBinary);

        private static readonly TokenListParser<LangToken, Expression> Comparand =
            Parse.Chain(Add.Or(Subtract), Term, MakeBinary);

        private static readonly TokenListParser<LangToken, Expression> Comparison =
            Parse.Chain(Lte.Or(Neq).Or(Lt).Or(Gte.Or(Gt)).Or(Eq), Comparand, MakeBinary);

        private static readonly TokenListParser<LangToken, Expression> Conjunction =
            Parse.Chain(And, Comparison, MakeBinary);

        private static readonly TokenListParser<LangToken, Expression> Disjunction =
        ...
        private static readonly TokenListParser<LangToken, Statement> RegularAssignment =
            from target in ReferenceAccess
            from eq in Token.EqualTo(LangToken.Equal)
            from expr in Expression
            select (Statement) new AssignmentStatement((ReferenceAccessItem)target, expr);

        private static readonly TokenListParser<LangToken, Statement> OperatorAssignment =
            from identifier in Identifier
            from eq in Increment.Or(Decrement)
            select (Statement) new AssignmentOperatorStatement(eq, identifier);
        ...
        private static readonly TokenListParser<LangToken, Statement> IfStatement =
        ...
        public static readonly TokenListParser<LangToken, Statement>
            SingleStatement = FunctionStatement.Try().Or(ConditionalStatement).Or(AssignmentStatement).Or(ForLoop)
                .Or(Return);

        private static readonly TokenListParser<LangToken, PrimitiveType> Int = Token.EqualTo(LangToken.Int)
            .Value(PrimitiveType.Int);

        private static readonly TokenListParser<LangToken, PrimitiveType> Char = Token.EqualTo(LangToken.Char)
            .Value(PrimitiveType.Char);

        private static readonly TokenListParser<LangToken, PrimitiveType> Void = Token.EqualTo(LangToken.Void)
            .Value(PrimitiveType.Void);
        ...
        public static readonly TokenListParser<LangToken, InitializationExpression> InitializationExpression =
            DirectInit.Or(ListInit);
        ...
        public static readonly TokenListParser<LangToken, DeclaratorAndInitializer> DeclaratorAndInitializer =
            from dec in ReferenceAccess
            from init in Initializer.OptionalOrDefault()
            select new DeclaratorAndInitializer((ReferenceAccessItem)dec, init);
        ...
        public static readonly TokenListParser<LangToken, IEnumerable<DeclarationStatement>> DeclarationStatement =
            ...
            from sm in Token.EqualTo(LangToken.Semicolon)
            select declsAndInits.Select(x => MakeDeclaration(type, x));

        private static DeclarationStatement MakeDeclaration(PrimitiveType type, DeclaratorAndInitializer x)
        {
            var fullType = MakeType(type, x.ReferenceAccessItemAccess);

            return new DeclarationStatement(fullType, x.ReferenceAccessItemAccess.Reference, x.Initialization);
        }

        private static ReturnType MakeType(PrimitiveType type, ReferenceAccessItem xReferenceAccessItemAccess)
        {
            return Ast.ReturnType.Create(type, xReferenceAccessItemAccess.IndirectionLevel,
                xReferenceAccessItemAccess.AccessExpression != null);
        }
        ...
        public static readonly TokenListParser<LangToken, ReturnType> ReferenceType =
            from b in BaseType
            from asterisks in Token.EqualTo(LangToken.Asterisk).Many()
        ...
        private static ReturnType GetRefType(PrimitiveType type, int indirectionLevel)
        {
